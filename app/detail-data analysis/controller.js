...
const Detaildataanalysis = require("./model");
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new Detaildataanalysis({...payload, image_url: filename})
                    await product.save()
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await Detaildataanalysis.findById(id);
                    ...
                    product = await Detaildataanalysis.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await Detaildataanalysis.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const index = async (req, res, next) => {
    try {
        ...
        let count = await Detaildataanalysis.find().countDocuments();

        let product = await Detaildataanalysis
        .find(criteria)
        .skip(parseInt(skip))
...
const destroy = async (req, res, next) => {
    try {
        let product = await Detaildataanalysis.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
