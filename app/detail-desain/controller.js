...
const Detaildesain = require("./model");
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new Detaildesain({...payload, image_url: filename})
                    await product.save()
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await Detaildesain.findById(id);
                    ...
                    product = await Detaildesain.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await Detaildesain.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const index = async (req, res, next) => {
    try {
        ...
        let count = await Detaildesain.find().countDocuments();

        let product = await Detaildesain
        .find(criteria)
        .skip(parseInt(skip))
...
const destroy = async (req, res, next) => {
    try {
        let product = await Detaildesain.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
